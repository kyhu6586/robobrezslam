...
from breezyslam.sensors import VelodynePuck, URG04LX

#Set to desired LIDAR device, depends on what was imported from breezyslam.sensors
lidarDevice = URG04LX
...
def load_data(datadir, dataset):
    ...
    while True:  
        ...
        #       time s      x                   y           theta
        delta = timestamp, float(toks[1]), float(toks[2]), float(toks[3])
        lidar = [int(tok) for tok in toks[24:]]
        timestamps.append(timestamp)
        scans.append(lidar)
        deltas.append(delta)
    fd.close()      
    return timestamps, scans, deltas

class MinesLaser(lidarDevice):
    
    def __init__(self):
        lidarDevice.__init__(self,0, 0)
        
# Class for MinesRover custom robot ------------------------------------------

class Rover(WheeledVehicle):
    def __init__(self):
        WheeledVehicle.__init__(self, 77, 100)                        
    def __str__(self):
        return '<%s ticks_per_cycle=%d>' % (WheeledVehicle.__str__(self), self.ticks_per_cycle)
    def computePoseChange(self, odometry):        
        return WheeledVehicle.computePoseChange(self, odometry[0], odometry[1], odometry[2], odometry[3])
    def extractOdometry(self, timestamp, x_pose, y_pose, theta_pose):
